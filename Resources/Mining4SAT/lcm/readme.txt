...
% exec_lcm [commands] input-filename support output-filename [options]

You have to give the commands in the first parameter, but the options
have to locate at the end. The separator of the items is " " (blank,
space). If you want to use other character as a separator, use
"sep_lcm". The usage is 

% sep_lcm separator [commands] input-filename support output-filename [options]
...
2. LCM (occ, ii, t_new, buf)
  ...
  (the buffer and ID of created transactions start from buf, and t_new
   ...
   --- in the case of maximal itemset mining, compute the union of the suffixes of transactions to be merged, and compute the weight of each item (item weight of item i is the sum of weights of transactions to be merged including i).
