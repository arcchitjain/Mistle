...
for data, class_vars in dataset_class_vars.items():
    ...
    # res_path = "./Output/xps/classify/ecoli-all-1d-pop-cccp-20200910090701/"
    # krimp_item_dict = {
    #     0: 13,
    #     1: 11,
    #     2: 15,
    ...
    for fold in range(1, 11):
        ...
        #     minsup=rel_minsup, k=10000, lossy=False
        # )
        ...
        #
        # mistle_topk_time += time() - start_time
        # if fold_accuracy is not None:
        #     mistle_topk_fold_accuracy_list.append(fold_accuracy)
        #     mistle_topk_attempted_list.append(
        #         len(attempted_test_pos) + len(attempted_test_neg)
        #     )
        ...
        if len(train_neg) < len(train_pos):
            mistle_pos_theory, _ = Mistle(train_neg, train_pos).learn(
                minsup=max(round(rel_minsup * len(train_pos)), 1), lossy=False
            )
            if len(get_clauses(mistle_pos_theory)) == 0:
                mistle_neg_theory, _ = Mistle(train_pos, train_neg).learn(
                    minsup=max(round(rel_minsup * len(train_neg)), 1), lossy=False
                )
            else:
            ...
            if len(get_clauses(mistle_neg_theory)) == 0:
                mistle_pos_theory, _ = Mistle(train_neg, train_pos).learn(
        ...
        fold_accuracy, attempted_test_pos, attempted_test_neg = classify_by_satisfiability_both(
            mistle_pos_theory, mistle_neg_theory, test_pos, test_neg, default_class
        )
    ...
    # Mistle - topk
    # print("mistle_topk_fold_accuracy_list = " + str(mistle_topk_fold_accuracy_list))
    # print("mistle_topk_attempted_list = " + str(mistle_topk_attempted_list))
    # weighted_sum = 0
    # for m, a in zip(mistle_topk_fold_accuracy_list, mistle_topk_attempted_list):
    #     weighted_sum += a * m
    # print("weighted sum = " + str(weighted_sum))
    # mistle_topk_attempted = sum(mistle_topk_attempted_list)
    # mistle_topk_accuracy = weighted_sum / mistle_topk_attempted
    # print("weighted_mistle_topk_accuracy = " + str(mistle_topk_accuracy))
    # print("mistle_topk_atempted = " + str(mistle_topk_attempted))
    # print("mistle_topk_coverage = " + str(mistle_topk_attempted / nb_rows))
    # print(
    #     "naive_mistle_topk_attempted_list = "
    #     + str(sum(mistle_topk_fold_accuracy_list) / len(mistle_topk_fold_accuracy_list))
    # )
    # print("Mistle - TOPK MDL Selections = " + str(mistle_topk_mdl_list))
    # mistle_topk_accuracy_dict[data] = (
    #     mistle_topk_accuracy,
    #     mistle_topk_attempted,
    #     mistle_topk_attempted / nb_rows,
    # )
